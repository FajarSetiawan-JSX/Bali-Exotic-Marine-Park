...
const props = defineProps(['divisions', 'total']);
...
async function getSpv() {
    try{
        ...
        const response = await axios.get('/api/priority-first/attach');
        supervisors.value = response?.data?.data;
    }catch(error){
        eror(`Error! Code: ${error?.response?.status}`, error?.response?.statusText);
...
<template>
    ...
    <AuthenticatedLayout>
                ...
                <template v-else>
                    <template v-if="supervisors.length > 0">
                        <div class="grid grid-cols-1 gap-4">
                            <div data-aos="fade-up" data-aos-anchor-placement="center-bottom" data-aos-offset="0" :data-aos-delay="index * 400" v-for="(supervisor, index) in supervisors" :key="index" class="p-5 rounded-xl outline-1 outline-slate-200 bg-white">
                                <div class="flex justify-between items-center">
                                    <div>
                                        ...
                                        <div class="flex justify-between items-center my-0.5">
                                            <div v-if="supervisor.teams.length > 0" class="flex -space-x-2">
                                                <img v-for="(team, index) in supervisor.teams" :key="team.id"
                                                :src="`https://ui-avatars.com/api/?name=${team.division}&background=7DD3fc&color=${team.color}`"
                                                title="image"
                                                class="w-8 h-8 rounded-full border-2 border-white hover:-translate-y-1 transition-all duration-300">

                                            </div>
                                            <span class="mx-2 text-slate-600 text-xs">{{ supervisor.teams.length }} Divisions</span>
                                        </div>
                                    </div>
                                ...
                                <Transition name="collapse">
                                    <div v-if="dropdown === supervisor.id" class="ml-5 my-5">
                                        <template v-if="supervisor.teams.length > 0">
                                            <div class="p-3.5 grid grid-cols-1 gap-3">
                                                <div v-for="(team, index) in supervisor.teams" :key="index" class="flex justify-between items-center">
                                                    <div class="flex">
                                                        <div class="font-bold font-second text-3xl px-1 border-r-2 border-indigo-500">
                                                            {{ index + 1 }}
                                                        </div>
                                                        <div class="mx-1.5">
                                                            <h6 class="text-black text-lg font-semibold">{{ team.division }}</h6>
                                                            <span class="text-slate-400 text-xs font-primary">{{ team.user }} users</span>
                                                        </div>
                                                    </div>
        ...
        <Create v-if="opencreate" :divisions="props.divisions" :users="props.total" @close="opencreate = false" />
    </AuthenticatedLayout>
</template>
<style scoped>
