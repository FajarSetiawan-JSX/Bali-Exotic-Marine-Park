...
async function hapus() {
    try{
        loading.value = true;
        const response = await axios.delete(`/api/priority-first/Fulltime/${props?.division?.id}`);
        emit('success')
    }catch(error){
...
<template>
    <div @click="emit('close')" class="fixed inset-0 z-[9999] flex justify-center items-center bg-gradient-to-br from-indigo-950/95 via-violet-900/85 to-fuchsia-900/90 backdrop-blur-sm">
        <section @click.stop data-aos="zoom-in" class="p-3 rounded-lg border-2 border-slate-300 bg-black mx-3 max-w-md">
            ...
            <p data-aos="fade-down" data-aos-delay="500" class="text-center text-sm text-white/80">Are you sure you want to remove the <span class="text-white">{{ props?.division?.name }}</span> division from this schedule? This acction can't be undone and will unassign all related staff</p>
