...
async function create() {
    try{
        err.value = {};
        loading.value = true;
        const response = await axios.post('/api/priority-first/Shifting', form.value);
        emit('success')
    }catch(error){
...
<template>
    <div class="fixed inset-0 z-[9999] flex items-center justify-center bg-gradient-to-br from-indigo-950/95 via-violet-900/85 to-fuchsia-900/90 backdrop-blur-sm p-4">
        <div data-aos="zoom-in" class="flex max-h-max w-full max-w-3xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
                ...
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500">
                            create a new work session for shifting work hours
                        </p>
                    </div>
            ...
            <form @submit.prevent="create" class="flex flex-1 flex-col overflow-hidden font-second">
                <div class="flex-1 overflow-y-auto max-h-max p-6">
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div class="col-span-2">
                            <label for="name" class="block text-sm font-medium text-slate-700">Name</label>
                            <input v-model="form.name" type="text" name="name" id="name" placeholder="Exp: Shift 1" class="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm shadow-sm placeholder:text-slate-400 focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500" />
