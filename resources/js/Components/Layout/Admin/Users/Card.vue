...
import { timeNow } from '@/Helper/DateNow'
...
const active = computed({
    get: () => props.user.status === 1,
    set: (value) => {
        props.user.status = value ? 1 : 0;
    }
});
async function toggle() {
    try{
        loading.value = true;
        const response = await axios.patch(`/api/priority-first/user/status/${props?.user?.id}`, {
            status: active.value
        })
        succes('Success',response?.data?.message);
    }catch(error){
    ...
    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
        ...
        <div class="flex justify-between">
            <div class="flex gap-4">
                <div class="relative">
                    <img :src="`https://ui-avatars.com/api/?name=${props?.user?.name}&background=2563EB&color=ffffff&rounded=true`" class="h-16 w-16 rounded-2xl object-cover">
                    <span v-if="props?.user?.online" class="absolute bottom-0 right-0 h-3.5 w-3.5 rounded-full border-2 border-white bg-emerald-500"/>
                </div>
                <div>
            ...
            <div ref="menu" class="relative">
                ...
                <Transition name="zoom">
                    <div v-if="showMenu" class="absolute right-0 top-10 z-20 w-40 overflow-hidden rounded-xl border bg-white shadow-xl">
                        <button type="button" @click="emit('update', props?.user)" class="flex w-full items-center gap-3 px-4 py-3 text-sm hover:bg-slate-50">
                            <Pencil :size="16"/>
                            Edit
                        </button>
        ...
        <div class="mt-6 grid grid-cols-2 gap-y-4 text-sm">
            ...
            <span class="text-right font-medium">
                {{ timeNow(props?.user?.join) }}
            </span>
        </div>
        <hr class="my-6">
        <!-- Footer -->
        <div class="flex items-center justify-between">
            <span :class="props.user.status ? 'text-emerald-600' : 'text-red-500'" class="font-medium ">
                {{ props?.user?.status == 1 ? 'Active' : 'Non active' }}
            </span>
            <label class="relative inline-flex cursor-pointer">
                <input :disabled="loading" v-model="active" @change="toggle" type="checkbox" class="peer sr-only">
