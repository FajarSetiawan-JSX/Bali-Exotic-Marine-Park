...
const props = defineProps(['divisions', 'positions', 'user']);
...
async function update() {
    try{
        err.value = {};
        loading.value = true;
        const response = await axios.patch(`/api/priority-first/user/${form?.value.id}`, form.value);
        emit('success');
        reset();
    }catch(error){
...
watch(()=>props.user, (newuser)=>{
    if(newuser){
        ...
        form.value.nik = newuser?.nik;
        form.value.email = newuser?.email;
        form.value.gender = newuser?.gender;
        form.value.birthday = newuser?.birthday;
        form.value.phone = newuser?.phone;
        form.value.emergency = newuser?.emergency;
        form.value.city = newuser?.city;
        form.value.address = newuser?.address;
        form.value.division = newuser?.division_id;
        form.value.position = newuser?.position[0]?.id;
    }
},{immediate:true})
onMounted(()=>{
    setTimeout(()=>{
